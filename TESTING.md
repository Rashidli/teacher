...
(test datası yaradacaqsan; şəkillər baza nüsxəsində yoxdur).

> **Diqqət:** `php artisan test` elə bu qovluqda işləyir (ayrıca staging yoxdur). Baza
> təhlükəsizdir (sqlite `:memory:`), fayl sistemi isə `Tests\TestCase`-dəki
> `Storage::fake('public')` ilə qorunur. Real diskə toxunan test yazmayın.
